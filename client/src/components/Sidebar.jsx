...
function Sidebar({onlinePeopleExcludeSelf, selectedUserId, changeSelectedUserId, offlinePeople, id, username, logout, friends}) {
    ...
    return (
        <>
            <div className={"absolute z-20 w-screen bg-white p-5 transition-all duration-300 flex items-center dark:bg-neutral-800 " + (open && "-translate-y-full")}>
            ...
            <div className={"bg-white pt-6 flex flex-col max-w-sm transition-all duration-300 sm:absolute z-10 sm:h-full sm:w-3/4 h-full dark:bg-neutral-800 " 
            + (!open ? " absolute -translate-x-full w-0 opacity-0" : "w-1/3")}>
                ...
                {settings ? <Settings logout={logout}/> :
                    <div className='h-full w-full'>
                        ...
                        className={"flex gap-2 border-b border-gray-100 dark:border-neutral-700 pl-5 py-3 w-full "  + (friendRequestPage && "bg-gray-100 dark:bg-neutral-700")}>
